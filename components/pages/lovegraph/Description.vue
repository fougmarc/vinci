<template lang="pug">
  .root
    transition(appear @enter='enter')
      .inner
        p.subtitle.description-fade-in
          | Le dialogue, le leadership , la solidarité, la cohesion sociale,la paix 
        p.subtitle.description-fade-in
          | la democraticie, la liberté d'expression, la discipline, la loyauté, l'unicité
        p.item.description-fade-in
          | la justice, la dignité, la liberté, l'ordre, l'egalité, la securité, le progres
        p.item.description-fade-in
          | Le respect de l'Etat, la non violence, la rigueur, l'Intellect, la valorisation de la femme 
       
</template>

<script>
export default {
  methods: {
    async enter (el, done) {
      await this.$delay(800);
      requestAnimationFrame(() => {
        TweenMax.staggerTo('.description-fade-in', 0.8, {
          ease: 'ease',
          startAt: { opacity: 0, y: 20, },
          opacity: 0.9, y: 0,
          onComplete() { done() }
        }, 0.1)
      });
    },
  }
}
</script>

<style lang="stylus" scoped>
.root
  letter-spacing 3px
  text-align left

.inner
  padding 0 20px
  position relative
  +pc-only()
    padding 0 15%

.title
  font-size 18px
  line-height 0.6em
  position relative
  +pc-only()
    font-size 24px

span.underline
  text-decoration: none;
  border-bottom: 1px solid
  padding-bottom: 9px;
  padding-right -10px

.subtitle
  font-size 12px
  padding-top 5px
  padding-bottom 2em

.item
  font-size 12px
  line-height 2em
  position relative
  +pc-only()
    font-size 15px

.description-fade-in
  opacity 0
</style>
